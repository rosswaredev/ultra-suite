...
describe("HabitDetailPresenter", () => {
  ...
  it("should update target count", () => {
    const { habitDetailPresenter } = setup();

    habitDetailPresenter.updateTargetCount(3);

    expect(habitDetailPresenter.targetCount).toBe(3);
  });

  it("should update target period", () => {
    const { habitDetailPresenter } = setup();

    habitDetailPresenter.updateTargetPeriod(4);

    expect(habitDetailPresenter.targetPeriod).toBe(4);
  });
});
